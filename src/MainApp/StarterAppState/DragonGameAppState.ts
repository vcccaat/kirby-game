...
export class DragonGameAppState extends StarterAppState {
  ...
  handleSceneGraphSelection(m: any) {
    ...
    console.log(m);
    console.log(
      `Transform with position:${m.transform.position}\nrotation: ${
  ...
  addArmModel() {
    ...
    let radius = 5;
    ringModel.segments = [
      new RingSegment(joints[0], joints[1], radius, [
        Color.FromString("#ff0000"),
        Color.FromString("#00ff00"),
      ]),
      new RingSegment(joints[1], joints[2], radius, [
        Color.FromString("#00ff00"),
        Color.FromString("#0000ff"),
      ]),
      new RingSegment(joints[2], joints[3], radius, [
        Color.FromString("#0000ff"),
  ...
  updateSpinningArms(t: number) {
    ...
    for (let a of arms) {
      ...
      let sb = 5;
      let v2 = V3(
        Math.sin(t * sa) * armlen,
        Math.cos(t * sa) * armlen,
        50 + armlen
      );
      let v3 = V3(Math.sin(t * sb) * armlen, Math.cos(t * sb) * armlen, 0).plus(
        v2
      );
      a.segments[1].end = v2;
  ...
  async initDragonGame(startInGameMode: boolean = true) {
    ...
    this.addModelFromFile(
      ...
      new NodeTransform3D(
        V3(100, 100, 80),
        Quaternion.FromAxisAngle(V3(1, 0, 0), -Math.PI * 0.5).times(
          Quaternion.FromAxisAngle(V3(0, 0, 1), -Math.PI * 0.5)
        ),
        V3(1, 1, 1).times(0.1)
      )
    );
    ...
    this.gameSceneController.camera.setPose(
      new NodeTransform3D(
        ...
        new Quaternion(
          -0.48287245789277944,
          0.006208070367882366,
          -0.005940267920390677,
          0.8756485382206308
        )
      )
    );
