...
export class SVGLoader extends Loader {
	...
	async load(url: string, onLoad: (data: SVGParsedData) => void = undefined, onProgress = undefined, onError = undefined) {
		const scope = this;
	...
	parse(text: string): SVGParsedData {
		...
		*/
		...
		function parseStyle(node, style) {
			...
			function addStyle(svgName: string, jsName: string, adjustFunction?: (v: any) => number) {
		...
		const rootSvgNode = parseNode(xml.documentElement as unknown as SVGElement, {
			fill: '#000',
			stroke: undefined,
			opacity: 1,
			fillOpacity: 1,
		...
		const data: SVGParsedData = { paths, rootSvgNode: rootSvgNode, xml: xml.documentElement };
