...
import { ASceneController, ASceneControllerOptions, ASceneModel, ASceneNodeModel, ALoadedModel, ASelectionModel, AMVCMap } from './index';
...
export abstract class AAppState<NodeModelType extends ASceneNodeModel, SceneModelType extends ASceneModel<NodeModelType>> extends AObject {
	...
	set currentNewModelTypeName(value: string) {
		this._currentNewModelTypeName = value;
	}
	// @ts-ignore
	...
	AppComponent(
		sceneControllerClass: ClassInterface<ASceneController<NodeModelType, SceneModelType>>,
		name: string,
		classMap: AMVCMap,
		...
		return function ReturnAppComponent(props: any) {
			...
			return (
				<div className='canvas'>
					<div className='anigraphcontainer' ref={container} tabIndex={options.sceneNumber} key={options.sceneNumber ? options.sceneNumber : uuidv4()}>
						{props.children}
					</div>
				</div>
			);
