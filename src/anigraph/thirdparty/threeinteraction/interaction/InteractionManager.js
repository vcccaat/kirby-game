...
// function ContainsListener(node, type) {
//     switch (type) {
//         case "pointertap":
//             if ('mouse')
//
...
class InteractionManager extends EventDispatcher {
     ...
     * @param {Scene} scene - A reference to the current scenevis
    ...
    /**
     * This function is provides a neat way of crawling through the scenevis graph and running a
    ...
    //   // if(interactionEvent.type == "mousedown"){
    ...
    //     //TODO Note AbeCHANGE? why the order of children backwards for 2D where they are same depth...
    //     for (let i = children.length - 1; i >= 0; i--) {
    ...
    //
    //         if (childHit) {
    //           //ABE CHANGE TODO: Look into whether this should be put back in some form
    //           // It was preventing siblings that are both interactive from both firing... I think it would also prevent interactions of e.g. children with interactions...
    ...
    //
    //     //ABE CHANGE TODO: why would interactionEvent.target already being hit matter? This would mean only one hit... grrrr.....
     ...
     * @param func
     * @param hitTest
     * @param interactive - override whether displayObject.interactive is true. This makes sense e.g., if you are testing
     * the child of an interactive node
     * @returns {boolean}
     */
    processInteractive(interactionEvent, displayObject, func, hitTest, interactive) {
        if (!displayObject || !displayObject.visible) {
        ...
        interactive = displayObject.interactive || interactive;
        let hit = false; // has a hit been detected
        let interactiveParent = interactive;

        let intersects = interactionEvent.intersects;
        const respondants = {};

        function findRespondingAncestors(intersection, currentNode, ancestorList) {
            // if(currentNode.interactive && containsListener(currentNode, interactionEvent.type){
            if (currentNode.interactive && respondants[currentNode.uuid] == undefined) {
                ancestorList.push(currentNode);
                respondants[currentNode.uuid] = currentNode;
            }
            if (currentNode.parent) {
                return findRespondingAncestors(intersection, currentNode.parent, ancestorList);
            }else{
                return ancestorList;
            }
        ...
        if (intersects.length < 1) {
            ...
            for (let isect of intersects) {
                hit = true;
                hitResponses.push(findRespondingAncestors(isect, isect.object, []));
            }
        }
        ...
        //     console.log('mouse down');
        // }
        // if(interactionEvent.type == 'mouseup'){
        //     console.log('mouse up');
        // }
        for (let iR = 0; iR < hitResponses.length; iR++) {
            let ilist = hitResponses[iR];
            // If any response in the first intersect sets blockOccludedIntersects to true, then we will skip subsequent
            // intersections
            if(!(interactionEvent.blockOccludedIntersects && iR>1)) {
                interactionEvent.triggeringIntersectionIndex=iR;

                for (let iN = 0; iN < ilist.length; iN++) {
                    let inode = ilist[iN];
                    interactionEvent.data.target = interactionEvent.target = inode;
                    if (func) {
                        func(interactionEvent, inode, hit);
                    }
                }
            }
        }
    ...
    onPointerMove(originalEvent) {
        ...
        if (events[0].pointerType === 'mouse') {
            ...
            // ABENOTE for parents interactive object (border order issue)
        }
    }
    ...
    configureInteractionEventForDOMEvent(interactionEvent, pointerEvent, interactionData) {
        ...
        // this.raycaster.setFromCamera(interactionData.global, this.camera);
        // let projI = this.camera.projectionMatrix.invert();
        //Abe add...
        if(Math.abs(this.camera.projectionMatrix.elements[11])>0.01){
            //perspective
            this.raycaster.ray.origin.setFromMatrixPosition(this.camera.matrixWorld);
            this.raycaster.ray.direction.set(interactionData.global.x, interactionData.global.y, 0.5).unproject(this.camera).sub(this.raycaster.ray.origin).normalize();
        }else{
            //ortho
            this.raycaster.ray.origin.set(interactionData.global.x, interactionData.global.y, -1).unproject( this.camera ); // set origin in plane of camera
            this.raycaster.ray.direction.set( 0, 0, - 1 ).transformDirection( this.camera.matrixWorld );
        }
        this.raycaster.camera = this.camera;


        // Not really sure why this is happening, but it's how a previous version handled things ABENOTE: there should be remove
    ...
    normalizeToPointerData(event) {
        ...
        if (this.supportsTouchEvents && event instanceof TouchEvent) {
            for (let i = 0, li = event.changedTouches.length; i < li; i++) {
                ...
                // ABENOTE: Remove these, as layerX/Y is not a standard, is deprecated, has uneven
